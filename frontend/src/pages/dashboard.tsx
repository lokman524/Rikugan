...
const DashboardPage: React.FC = () => {
    ...
    const mockBounties: Bounty[] = [
      {
        ...
        description: 'There is a critical bug in the login system that prevents users from accessing their accounts. Need to investigate and fix the JWT token validation.',
        bountyAmount: 250.00,
      ...
      },
      {
        ...
        description: 'Add dark mode support to the entire application. This includes updating all components to respect the theme preference.',
        bountyAmount: 150.00,
      ...
      },
      {
        ...
        description: 'Optimize database queries for better performance. Focus on the user and task tables that are experiencing slow response times.',
        bountyAmount: 300.00,
      ...
      },
      {
        ...
        description: 'Design and implement a comprehensive user profile page where users can view and edit their information.',
        bountyAmount: 200.00,
      ...
      },
      {
        ...
        description: 'Write comprehensive API documentation for all endpoints. Include examples and error handling scenarios.',
        bountyAmount: 100.00,
  ...
  useEffect(() => {
    ...
    if (priorityFilter !== 'ALL') {
      filtered = filtered.filter(bounty => bounty.priority === priorityFilter);
    }
  ...
  const handleTakeTask = (bountyId: string) => {
    ...
    setBounties(prevBounties =>
      prevBounties.map(bounty =>
        bounty.id === bountyId
          ? { ...bounty, status: 'IN_PROGRESS' as const, assignedTo: user?.username }
          : bounty
      )
    );
  };

  const availableBounties = filteredBounties.filter(b => b.status === 'AVAILABLE' || b.assignedTo !== user?.username);
  const myTasks = filteredBounties.filter(b => b.assignedTo === user?.username);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex">
      ...
      <div className="flex-1 lg:ml-0 transition-all duration-300">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Available Tasks</p>
                  <p className="text-2xl font-semibold text-gray-900 dark:text-white">{availableBounties.length}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900 dark:text-white">{myTasks.length}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900 dark:text-white">${user?.balance?.toFixed(2) || '0.00'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                <div className="p-2 bg-purple-100 dark:bg-purple-900 rounded-lg">
                  <svg className="w-6 h-6 text-purple-600 dark:text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 20 20">
          ...
          {myTasks.length > 0 && (
            <div className="mb-8">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">My Active Tasks</h2>
          ...
          {/* Available Bounties */}
          <div>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Available Bounties</h2>
            {filteredBounties.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow text-center">
                <p className="text-gray-500 dark:text-gray-400">No bounties found matching your criteria.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          {selectedBounty && (
            <>
              <ModalHeader className="flex flex-col gap-1">
                ...
                <p className="text-sm text-gray-600">Created by {selectedBounty.createdBy}</p>
              </ModalHeader>
              <ModalBody>
                <div className="space-y-4">
                  <div>
                    <h3 className="font-semibold mb-2">Description</h3>
                    <p className="text-gray-700 dark:text-gray-300">{selectedBounty.description}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <h4 className="font-semibold">Bounty Amount</h4>
                      <p className="text-2xl font-bold text-success">${selectedBounty.bountyAmount.toFixed(2)}</p>
                    </div>
                    <div>
              ...
              <ModalFooter>
                ...
                {selectedBounty.status === 'AVAILABLE' && selectedBounty.assignedTo !== user?.username && (
                  <Button 
                    color="primary" 
                    onPress={() => {
